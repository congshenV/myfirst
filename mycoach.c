/*20150823 客运第一版，把出租定位部分删除，移植客运算法*/
#include <stdlib.h> 
#include <stdio.h> 
#include <errno.h> 
#include <string.h> 
	...
	{	
		...
		{ 
			case 0x01:    /*OBU*/
				...
				if(difftime(time(NULL),jinchuzhan_beat) >= 150)
				{
...
void *send_msg()
{
	...
	{
		/*日志删除部分*/
		if(difftime(time(NULL),first) >= 60)
		{  
		...
		first = time(NULL);
		//getTime(rmlog, 4);
		//sprintf(out,"rm /root/%s.log",rmlog);
		//system(out);
		}
...
void *rec_ask()
{
	...
	{
		/*心跳时间30分钟*/
		if(difftime(time(NULL),heartbeat_t) >= 30)   
		{
			heartbeat_t  = time(NULL);
			RSU_serial_num++;
		...
		int	col = 0;
		int ret_value=0;		
		ret_value= sqlite3_exec( db, "select * from postable limit 1;", local_callback, "fuck", &pErrMsg);
...
#define AP						6
#define AP1						7
#define Pos138_Times			8	//定出138次数
#define Unget_Times			    9	//定不出位置的次数
#define Atemp					10
#define Atemp2					11
#define Atemp3					12
#define Apos					13
...
void *locate_process()
{
	...
	int aveflag=0,stopflag=0,temp2=0,temp3=0;
	...
	{
		for(j=0;j<=4;j++)
			memset(huadubuf[i][j],0,11);
	}
	for(i=0;i<=5;i++)
	{
		for(j=0;j<=8;j++)
			memset(huadudatebar[i][j],0,200);
	}
	while(1)
	{
		char **result;
		...
		memcpy(obu_id,result[12],16);
		printf("obu_id  = %s \n",obu_id);										
		for(i=0;i<(row+1)*col;i++)
			printf("****************result[%d]=%s\n",i,result[i]);		
		count = 12;
		memcpy(obu_id,result[count],16);//
		/*没有进入循环导致的bug*/
		while((count<(row+1)*col))
		{
			int pos=0;	
			int startpark_stopflag=0;
			int temp1=0;		
			int locate_j=0;
			int locate_k=0;
			int eff=3;
			...
			if(cmp_obu(obu_id, result[count], 3))
			{
				...
				locate_i=huadu[huadu_temp_obu_num][Ai];
				locate_p=huadu[huadu_temp_obu_num][AP];
				locate_p1=huadu[huadu_temp_obu_num][AP1];
				unget_t=huadu[huadu_temp_obu_num][Unget_Times];
				if(book[obu_last3]==0)
				{
					printf("**********进站进站**********");
					pos=001;
				...
				if(fire_flag>=0)
				{
					for(locate_j=0;locate_j<11;locate_j++)
					{
						huadubuf[huadu_temp_obu_num][locate_i][locate_j]=rssi_data[locate_j];
					}
					temp1=findmaxRssiaddr(huadubuf[huadu_temp_obu_num][locate_i]);
					//printf("**********locate_i=%d\n",locate_i);
					//printf("***findmaxRssiaddr=0x%x\n",temp1);
					locate_i++;						
					if(locate_i>=acc)
					{
						locate_i=0;
				...
				if(stopflag==1)
				{
					if(locate_p>locate_p1)      //每收集acc个数据再做判断
					{
						locate_p1=locate_p;							
						for(locate_k=0;locate_k<9;locate_k++)
							huadudatebar[huadu_temp_obu_num][locate_k][locate_p1]=(int)(getAverRssi(huadubuf[huadu_temp_obu_num],0x11+locate_k));							
						aveflag=1;
					}
				
					printf("data is done over,aveflag=1时开始定位\n");
						
					/****************定位以及判断条件******************/
					if(aveflag==1)
					{
						printf("**********aveflag=1 开始定位!!!!!\n");
						if(huadu[huadu_temp_obu_num][Astateflag]<3&&huadudatebar[huadu_temp_obu_num][R4][locate_p1]>0xAA||huadudatebar[huadu_temp_obu_num][R9][locate_p1]>0xAA)
						{
							pos=105;			//	***************************************
						}
						else if(huadu[huadu_temp_obu_num][Astateflag]<3&&huadudatebar[huadu_temp_obu_num][R2][locate_p1]>0xAA||huadudatebar[huadu_temp_obu_num][R8][locate_p1]>0xA0)										
						{
							if(huadudatebar[huadu_temp_obu_num][R2][locate_p1]-huadudatebar[huadu_temp_obu_num][R6][locate_p1]>0)										
							{
								pos=109;	 		//	***************************************
							}
							else pos=108;	 		//	***************************************
						}			 
						else if(huadu[huadu_temp_obu_num][Astateflag]<3&&huadudatebar[huadu_temp_obu_num][R6][locate_p1]>0xa0&&huadudatebar[huadu_temp_obu_num][R6][locate_p1]<0xB0&&huadudatebar[huadu_temp_obu_num][R2][locate_p1]<0xA7)										
						{
							pos=110;	 		//	***************************************
						}
						else unget_t++;				//**********************************************										
						...
						if(unget_t>=3)
						{
							unget_t=0;
							pos=138;
							huadu[huadu_temp_obu_num][Pos138_Times]++;
						}
						if(pos>0&&pos<130)
						{
							huadu[huadu_temp_obu_num][Astateflag]=2;
						}
						startpark_stopflag=(pos/10)%10;
						if(startpark_stopflag==3)
				...
				if(huadu[huadu_temp_obu_num][Pos138_Times]>5)
				{
					locate_i=0;locate_j=0;locate_k=0;locate_p=0;locate_p1=0;
					if(huadu[huadu_temp_obu_num][Astateflag]==3)
					{
						...
						for(j=0;j<=8;j++)
							memset(huadudatebar[huadu_temp_obu_num][j],0,200);
						for(j=0;j<=acc;j++)
							memset(huadubuf[huadu_temp_obu_num][j],0,11);
						book[obu_last3]=0;	
						writeFile(fop_log, obu_id,14);
				...
				huadu[huadu_temp_obu_num][Ai]=locate_i;
				huadu[huadu_temp_obu_num][AP]=locate_p;
				huadu[huadu_temp_obu_num][AP1]=locate_p1;
				huadu[huadu_temp_obu_num][Unget_Times]=unget_t;
				printf("\n>>>>>一轮OBU数据库数据处理完毕，[obu:%d]: Final Result: pos = %d\n",obu_last3,pos);
				if(pos!=0)
				{
					/*  将pos转换成字符串写入日志中*/
			...
			{		//****************************************************************************************************************************************
				...
				memcpy(obu_temp_id,result[count],16);
				obu_last3=atoi(obu_temp_id+13);	
				printf("obu de hou san wei int xing id  obu_last3=%d\n",obu_last3);
				for(k=0;k<=5;k++)
				{
					if(obu_last3==huadu[k][AOBU])
					{
				...
				if(stopflag==1)
				{
					...
					if(locate_p>locate_p1)      //每收集acc个数据再做判断
					{
						locate_p1=locate_p;							
						for(locate_k=0;locate_k<9;locate_k++)
							huadudatebar[huadu_temp_obu_num][locate_k][locate_p1]=(int)(getAverRssi(huadubuf[huadu_temp_obu_num],0x11+locate_k)+huadudatebar[huadu_temp_obu_num][locate_k][locate_p1-1]*(locate_p1-1))/locate_p1;							
						aveflag=1;
					}
					...
					if(aveflag==1)
					{
						...
						else if(huadu[huadu_temp_obu_num][Astateflag]<3&&huadudatebar[huadu_temp_obu_num][R6][locate_p1]<0xB0&&huadudatebar[huadu_temp_obu_num][R2][locate_p1]<0xA7)										
						{
							pos=110;	 		//	***************************************
						}
						else unget_t++;				//**********************************************										
...
ssize_t nread(int fd,unsigned char *ptr)
{
	...
	if(ptr[0] == 0x04)
	{
		...
		Serial_Id_Sql++;
		if (Serial_Id_Sql==0xFFFFFFF)/*数据库的ID*/
		{	
			Serial_Id_Sql=0;
		}
		...
		//字符串的路由id
		//	memcpy(route1_id,uart_convet+22,16);/*sql*/
		//	memcpy(route2_id,uart_convet+40,16);/*sql*/		
		//	memcpy(route3_id,uart_convet+58,16);/*sql*/
		//	memcpy(route4_id,uart_convet+76,16);/*sql*/

   		memcpy(obu_id,data_from_uart+1,8);	
		memset(SQL_Insert,0,200);
		sprintf(SQL_Insert,"insert into Rssi_table values(%d,'%s',%d,%d,%d,%d,%d,%d,%d,%d,'%s');",Serial_Id_Sql,obu_id_sql,route1_id,route1_val,route2_id,route2_val,route3_id,route3_val,route4_id,route4_val,time_sql);
	
		ret_sql = sqlite3_exec(db, SQL_Insert, NULL, NULL, &pErrMsg);  
...
getTime(curTime,0);
 switch(blog)
	...
	case 12:
		...
		break;	
	case 13:
		ret = fprintf(fp, "[%s]:POS=%s\n",curTime,str);
		break;	
	case 14:
		ret = fprintf(fp, "[%s]:OBU=%s\n",curTime,str);
		break;	
	case 15:
